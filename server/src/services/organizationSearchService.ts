...
const CURATED_INSTITUTIONS: Record<string, {
  ...
  'موبايلي': {
    ...
    overview: 'تُعد شركة موبايلي إحدى كبرى شركات الاتصالات الرائدة في المملكة والمنطقة، وتملك شبكة واسعة من الألياف الضوئية ومراكز البيانات المصنفة دولياً، وتقدم منظومة خدمات اتصالات متكاملة للأفراد وقطاع الأعمال.',
    ...
    technologies: ['شبكات الألياف الضوئية (FTTH)', 'شبكات الجيل الخامس (5G)', 'مراكز البيانات القياسية Tier-III/IV', 'حلول الحوسبة السحابية والربط المؤسسي']
  },
  'زين': {
...
function buildDeterministicAcademicText(
  ...
  const dept = department?.trim() || 'التخصص الأكاديمي';
  ...
  const paragraph2 = curated?.departments
    ...
    : `وتتسم طبيعة العمل الميداني داخل المنظومة بالتكامل المهني والتشغيلي المنسق بين مختلف أقسامها وإداراتها التخصصية. حيث تعتمد بيئة العمل على تطبيق منهجيات عمل مرنة ومعايير تشغيل قياسية، وتوظيف الأنظمة التقنية المؤتمتة وقواعد البيانات المركزية، مما يوفر للمتدرب فرصة حقيقية للتعامل مع بيئة إنتاجية حية تحاكي أعلى المقاييس المهنية العالمية وتصقل قدراته الإجرائية.`;
...
3. التعرّف على التقنيات والبرمجيات ومعايير الجودة المطبقة في قطاعات العمل الفعلية.
...
1. المهارات الفنية والتقنية التخصصية:
   - التطبيق العملي للأنظمة والبرمجيات المستخدمة لدى المنظومة، والتعامل مع البنى الرقمية وقواعد البيانات التشغيلية.
...
- التوصيات الموجهة للجامعة / الكلية: إدراج دراسات حالة تطبيقية مستمدة من مشاريع المؤسسات الوطنية الكبرى ضمن المقررات الدراسية، والتركيز على الأدوات التقنية الحديثة المتبعة في سوق العمل السعودي.
- التوصيات لزملائي المتدربين: المبادرة الدائمة بطرح الاستفسارات العلمية، التوثيق اليومي الدقيق لكافة المهام المنجزة، والحرص على بناء علاقات مهنية مستدامة مع فرق العمل الميدانية.`;
  ...
  return {
    ...
    keyFacts: entityFacts?.keyFacts || [
      ...
      'بيئة تدريبية مطابقة لمعايير الجودة والحوكمة المؤسسية',
