...
export async function buildFinalReportData(userId: number): Promise<FinalReportData> {
  ...
  const profile: ReportProfileDTO = {
    ...
    companyLogo: (profileRecord as any).companyLogo || '',
    institutionLogo: (profileRecord as any).institutionLogo || '',
    reportTemplate: ((profileRecord as any).reportTemplate as any) || 'royal',
    executiveSummary: (profileRecord as any).executiveSummary || '',
    challengesText: (profileRecord as any).challengesText || '',
    recommendationsText: (profileRecord as any).recommendationsText || '',
